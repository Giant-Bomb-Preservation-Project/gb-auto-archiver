...
def create_csv(hd_url):
    ...
    disc(f"```diff\n>>      [{i}] {vars['filename']}\n```")
    with open(f'{daily_dir}/upload.csv', 'w', newline='', encoding='utf-8') as f:
...
# ---------- run once for “today minus 2 days” ----------
...
yesterday = target_date  # keep original variable name used below
...
if shows == 0:
    ...
    proc = subprocess.Popen(
        ["ia", "upload", f"--spreadsheet={daily_dir}/upload.csv"],
        stderr=subprocess.STDOUT, stdout=subprocess.PIPE,
        encoding='utf-8', text=True
    )
    log = open(f'{daily_dir}/ia_upload_{yesterday}.log', 'w', errors='ignore')
    ...
    for item in os.listdir(daily_dir):
        if item.endswith((".mp4", ".csv")):
            os.remove(os.path.join(daily_dir, item))
